...
describe('Valida a criação de um novo pedido', function () {
	...
	test('deve criar um pedido com 3 itens e calcular o total do pedido', function () {
		...
		order.addItem(new Item(1, 'Mouse', 100, new Dimension(10, 10, 10, 0.9)), 1);
		order.addItem(new Item(2, 'Teclado', 300, new Dimension(10, 10, 10, 0.9)), 1);
		order.addItem(new Item(3, 'Monitor', 1100, new Dimension(10, 10, 10, 0.9)), 2);
	...
	test('ao criar um pedido, deve gerar um número único', function () {
		const order = new Order('19361862170');
		expect(order.id).toBe('202200000001');
	});

	test('não deve ser possível adicionar itens duplicados a um pedido', function () {
		const order = new Order('19361862170');
		order.addItem(new Item(1, 'Monitor', 1100, new Dimension(10, 10, 10, 0.9)), 2);
